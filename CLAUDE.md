...
GuildManager uses a comprehensive CSS custom properties (CSS variables) system that enables runtime theme customization without code changes. All colors are defined in HSL format in `app/globals.css`.
...
Access the complete design system showcase at `/theme-demo` which includes:
- Color palette display with CSS variable names
- All components in various states and variants
- WoW-specific component examples
- Live theme editing controls (simulated)
